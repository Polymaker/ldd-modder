...
namespace LDDModder.PaletteMaker.Rebrickable.Services
{
    public class SetService : RebrickableService
    {
            ...
            string ordering = null)
        {
            ...
            {
                request.AddParameter("page", page.Value);
                var requestResult = await Client.ExecuteAsync<PagedResult<Set>>(request);
                if (requestResult.IsSuccessful)
                    return requestResult.Data.Results;
            ...
            return await GetRequestAllPagesAsync<Set>(request);
        }
        ...
        public async Task<Set> GetSetAsync(string setNum)
        {
            var request = new RestRequest("lego/sets/{set_num}/", Method.GET, DataFormat.Json);
            request.AddUrlSegment("set_num", setNum);
            var requestResult = await Client.ExecuteAsync<Set>(request);
            if (!requestResult.IsSuccessful)
                throw new Exception(requestResult.ErrorMessage);
            return requestResult.Data;
        }
        ...
        public async Task<IEnumerable<SetPart>> GetSetPartsAsync(string setID, bool includeMinifigs = false, int? page = null)
        {
            var request = new RestRequest("lego/sets/{set_num}/parts/", Method.GET, DataFormat.Json);
            ...
            {
                request.AddParameter("page", page.Value);
                var requestResult = await Client.ExecuteAsync<PagedResult<SetPart>>(request);
                if (requestResult.IsSuccessful)
                    return requestResult.Data.Results;
            ...
            return await GetRequestAllPagesAsync<SetPart>(request);
        }
    }
}
