...
namespace LDDModder.LDD.Meshes
{
    public class MeshBuilder
    {
        ...
        private IEnumerable<Triangle> GetTangentFaces(Triangle triangle)
        {
            foreach(var tri in Triangles)
            {
                if (tri != triangle && tri.Vertices.Count(x => triangle.ContainsVertex(x.Position)) == 2 && Vector3.Distance(tri.Normal,triangle.Normal) < 0.3f)
                    yield return tri;
            }
        }
        ...
        private void GenerateRoundEdgeData()
        {
            ...
            for (int t = 0; t < Triangles.Count; t++)
            {
                ...
                var edges = BoundaryEdges.Where(x => triangle.Vertices.Any(v=>x.Contains(v))).ToList();

                for (int i = 0; i < 3; i++)
                {
                    var index = Indices[(t * 3) + i];
                    for (int j = 0; j < 6; j++)
                        index.RoundEdgeData.Coords[j] = new Vector2(1000, 1000);
                }
                ...
                if (edges.Any())
                {
                    ...
                    {
                        var coords = GetEdgeCoords(triangle, e);
                        if (!coords.Any(c => c.Y < 0))
                            goodEdges.Add(e);
                    }

                    for (int x = 0; x < Math.Min(goodEdges.Count, 6); x++)
                    {
                        var coords = GetEdgeCoords(triangle, goodEdges[x]);
                        for (int i = 0; i < 3; i++)
                        {
                            var index = Indices[(t * 3) + i];
                            int offset = x < 3 ? 1 : 0;
                            index.RoundEdgeData.Coords[((x * 2) + offset) % 6] = coords[i] * edgeWidthRatio;
                            index.RoundEdgeData.Coords[((x * 2) + offset) % 6].X += 100;
                            index.RoundEdgeData.Coords[((x * 2) + offset) % 6].X *= -1;
                        }
                    }
                }
        ...
        private Vector2[] GetEdgeCoords(Triangle triangle, Edge edge)
        {
            var coords = new Vector2[3];
            var plane = new Plane(triangle.GetCenter(), triangle.Normal);
            var p1 = plane.ProjectPoint(edge.P1.Position);
            var p2 = plane.ProjectPoint(edge.P2.Position);
            var vec = (p2 - p1).Normalized();
            
            plane.Origin = p1;

            for (int i = 0; i < 3; i++)
                coords[i] = plane.ProjectPoint2D(vec, triangle.Vertices[i].Position);

            if (coords.Any(x=>x.Y < 0))
            {
            }
            return coords;
        }
        ...
        public Mesh BuildMesh()
        {
            ...
            var roundEdgeDataDistinct = Indices.Select(x => x.RoundEdgeData).Distinct().ToList();
            int totalOffset = 0;
